  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="selector-group">
        <label class="selector-label">选择驾考辅导类型</label>
        <div class="style-selector">
          <button 
            v-for="etype in drivingExamTypeOptions" 
            :key="etype"
            class="style-option"
            :class="{ active: activeDrivingExamType === etype }"
            @click="activeDrivingExamType = etype"
          >
            {{ etype }}
          </button>
        </div>
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">报考驾照类型</label>
          <div class="style-selector">
            <button 
              v-for="ltype in licenseTypeOptions" 
              :key="ltype"
              class="style-option"
              :class="{ active: selectedLicenseType === ltype }"
              @click="selectedLicenseType = ltype"
            >
              {{ ltype }}
            </button>
          </div>
        ...
        <div class="selector-group">
          <label class="selector-label">当前备考阶段</label>
          <div class="style-selector">
            <button 
              v-for="stage in drivingStageOptions" 
              :key="stage"
              class="style-option"
              :class="{ active: selectedDrivingStage === stage }"
              @click="selectedDrivingStage = stage"
            >
              {{ stage }}
            </button>
          </div>
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          <label class="selector-label">输入具体的驾考科目、难点项目或练车问题</label>
          <div style="display: flex; gap: 0.5rem;">
            <button v-if="userInput" class="text-link-btn" @click="userInput = ''">清空输入</button>
            <button class="text-link-btn" @click="showDrivingTipsModal = true">驾考各科目扣分红线与看点参考指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
          placeholder="请描述您的驾考难点...（例如：C1手动挡科目二倒车入库总是压右边角，或者科目三靠边停车容易超过30cm，请提供死看点与修正技巧。）"
          style="min-height: 120px;"
        ></textarea>
        <div style="display: flex; justify-content: space-between; font-size: 0.75rem; color: var(--text-secondary);">
          <span>字符数: {{ userInput.length }} 字</span>
          <span>建议明确驾驶车型、具体操作项目与遇到的疑难细节</span>
        </div>
      </div>
      ...
      <div style="display: flex; gap: 0.75rem;">
        ...
        >
          {{ loading ? '正在精准拆解考点动作与编制通关方案中...' : '开始生成驾考通关与备考指南' }}
        </button>
    ...
    <section v-if="result || loading" class="glass-card">
      <div class="result-header">
        <span class="result-title">驾考通关与看点剖析方案</span>
      ...
      <div v-else-if="result">
        ...
        <div v-if="aiScores" class="scores-container" style="margin-bottom: 1.5rem; padding: 1.25rem; background: rgba(0,0,0,0.25); border-radius: 12px; border: 1px solid rgba(255,255,255,0.06);">
          <div style="font-weight: 700; font-size: 0.95rem; margin-bottom: 1rem; color: #a5b4fc; display: flex; justify-content: space-between; align-items: center;">
            <span>AI 驾考通关要领与扣分防范评估看板</span>
            <span style="font-size: 0.8rem; font-weight: normal; color: var(--text-secondary);">综合通关质量分: {{ getAverageScoreFromMap(aiScores) }} / 100</span>
          </div>
    ...
    <section v-if="showHistory" class="glass-card" style="margin-top: 1rem;">
      <div class="result-header">
        <span class="result-title">本地驾考指导历史记录</span>
      ...
      <div v-else class="history-grid" style="display: flex; flex-direction: column; gap: 0.75rem; max-height: 320px; overflow-y: auto;">
        <div v-for="item in historyList" :key="item.id" class="history-item" style="padding: 1rem; background: rgba(0,0,0,0.2); border-radius: 10px; border: 1px solid var(--card-border);">
          <div style="display: flex; justify-content: space-between; font-size: 0.8rem; color: var(--text-secondary); margin-bottom: 0.4rem;">
            <span>{{ item.timestamp }} · [{{ item.drivingExamType }} / {{ item.licenseType }} / {{ item.drivingStage }}]</span>
    ...
    <!-- 驾考各科目扣分红线与看点参考指南 Modal -->
    <div v-if="showDrivingTipsModal" class="modal-overlay" @click.self="showDrivingTipsModal = false">
      <div class="modal-content" style="max-width: 480px;">
        <h3>驾考各科目扣分红线与看点参考指南</h3>
        <p style="text-align: left; font-size: 0.825rem; margin-bottom: 1rem; color: var(--text-secondary);">
          帮助学员掌握各科目考核死穴与看点细节的核心准则：
        </p>
        <div class="modal-scroll-area" style="text-align: left; font-size: 0.825rem;">
          <div v-for="(tip, idx) in drivingTipsList" :key="idx" style="margin-bottom: 0.75rem; padding: 0.5rem 0.75rem; background: rgba(255,255,255,0.03); border-radius: 8px; border: 1px solid rgba(255,255,255,0.05);">
            <div style="color: var(--accent-color); font-weight: bold; margin-bottom: 0.2rem;">{{ tip.title }}</div>
            <div style="color: var(--text-primary); margin-bottom: 0.2rem;">考核重点: {{ tip.point }}</div>
            <div style="color: var(--text-secondary); font-size: 0.775rem;">防扣分建议: {{ tip.avoid }}</div>
          </div>
        </div>
        <button class="modal-btn" style="margin-top: 1rem;" @click="showDrivingTipsModal = false">关闭</button>
      </div>
    </div>
    ...
    <div v-if="showShareGuide" class="modal-overlay" @click.self="showShareGuide = false">
      <div class="modal-content">
        <h3>分享驾考学车与路考通关专家</h3>
        <p>扫码关注或将链接分享给准备考驾照的学员与驾校教练，助力一次通关。</p>
    ...
    <div v-if="showPrivacy" class="modal-overlay" @click.self="showPrivacy = false">
      <div class="modal-content">
        ...
        <div class="modal-text-content modal-scroll-area">
          <p>我们非常重视您的个人隐私。您在本应用中输入的驾考疑问与备考练习记录仅用于实时大模型生成，系统不会在云端永久存储或泄露您的个人信息。</p>
    ...
    <div v-if="showTerms" class="modal-overlay" @click.self="showTerms = false">
      <div class="modal-content">
        ...
        <div class="modal-text-content modal-scroll-area">
          <p>欢迎使用网腾无限 AI 驾校学车与路考通关专家。本工具生成的看点要领、防扣分技巧及理论口诀仅供学员练习与备考参考。</p>
          <p>在实际道路驾驶与驾考场地中，请严格遵守国家交通法律法规及当地考场与教练员的指导说明。</p>
        </div>
...
// 配置参数
const appTitle = ref(appConfig.title || '网腾无限AI - 驾校学车与路考通关专家');
...
const showDrivingTipsModal = ref(false);
...
// 4 种预设类型
const drivingExamTypeOptions = [
  '科目二倒车入库与侧方停车口诀',
  '科目三路考靠边停车与变道细节',
  '科目一与科目四理论错题速记',
  '驾考考前紧张心理心理安抚'
];
const activeDrivingExamType = ref(drivingExamTypeOptions[0]);

// 2 组属性：驾照类型 & 学习阶段
const licenseTypeOptions = ['C1手动挡', 'C2自动挡', 'A/B大型车辆'];
const selectedLicenseType = ref('C1手动挡');

const drivingStageOptions = ['零基础刚报名前', '场地训练期', '模拟考冲刺期'];
const selectedDrivingStage = ref('零基础刚报名前');

// 评估指标列表
const metricsList = [
  { key: 'drivingTechAccuracy', label: '驾驶要领准确度' },
  { key: 'examPointCoverage', label: '考点细节覆盖率' },
  { key: 'decontaminationTipsClarity', label: '防扣分技巧清晰度' },
  { key: 'psychologicalReassurance', label: '考前紧张疏导度' },
  { key: 'passRateOptimization', label: '通关率提升实用值' }
];
...
const drivingTipsList = [
  { 
    title: '科目二红线与细节', 
    point: '车身压线扣100分，中途停顿扣100分或5分；后视镜按身高调整，保持固定坐姿。', 
    avoid: '控制车速宜慢不宜停，打方向切忌犹豫迟缓，看点到达位置时要迅速转动方向盘。' 
  },
  { 
    title: '科目三路考红线与细节', 
    point: '变道/超车/靠边停车前打转向灯需满3秒，换挡切忌低头看挡，随时观察后视镜。', 
    avoid: '熄火后必须拉空挡再重新起步；遇路口提前减速踩刹车观察左右行人。' 
  },
  { 
    title: '坡道起步防熄火防溜车', 
    point: '溜车大于30cm扣100分；抬离合找半联动抖动点，听发动机声音变沉时松手刹踩油门。', 
    avoid: '切忌猛抬离合导致发动机猝熄；坡道定点前看准右前杠与定点线重合点。' 
  },
  { 
    title: '考前心态与紧张调理', 
    point: '考前腹式深呼吸3到5遍，将安全员视为路线导航指示，专注眼前当前操作动作。', 
    avoid: '切忌反复纠结上一项目是否被扣分，放宽心态，按平时的练车节奏稳步推进。' 
  }
];

// 计算纯结果文本 (剔除打分标签 [JIAXIAO_SCORES])
const displayResultText = computed(() => {
  if (!result.value) return '';
  return result.value.replace(/\[JIAXIAO_SCORES\][\s\S]*?\[\/JIAXIAO_SCORES\]/g, '').trim();
});

// 解析打分标签
const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[JIAXIAO_SCORES\](.*?)\[\/JIAXIAO_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('jiaxiao_history_records');
...
const saveHistory = () => {
  localStorage.setItem('jiaxiao_history_records', JSON.stringify(historyList.value));
};
...
const addHistoryRecord = () => {
  const newItem: HistoryItem = {
    ...
    drivingExamType: activeDrivingExamType.value,
    licenseType: selectedLicenseType.value,
    drivingStage: selectedDrivingStage.value,
...
const applyHistory = (item: HistoryItem) => {
  ...
  activeDrivingExamType.value = item.drivingExamType;
  if (item.licenseType) selectedLicenseType.value = item.licenseType;
  if (item.drivingStage) selectedDrivingStage.value = item.drivingStage;
...
const handleGenerate = async () => {
  ...
  try {
    const response = await fetch(apiEndpoint, {
      ...
      body: JSON.stringify({
        taskType: 'text',
        prompt: `任务指导: ${promptTopic.value}\n【驾考分类】: ${activeDrivingExamType.value}\n【驾照类型】: ${selectedLicenseType.value}\n【备考阶段】: ${selectedDrivingStage.value}\n【驾考疑难描述】: ${userInput.value}`,
        style: activeDrivingExamType.value
      })
    });
...
const handleApplyTemplate = (payload: {
  ...
  if (payload.drivingExamType) activeDrivingExamType.value = payload.drivingExamType;
  if (payload.licenseType) selectedLicenseType.value = payload.licenseType;
  if (payload.drivingStage) selectedDrivingStage.value = payload.drivingStage;
  if (inputCardRef.value) {
    inputCardRef.value.scrollIntoView({ behavior: 'smooth', block: 'center' });
  }
};
